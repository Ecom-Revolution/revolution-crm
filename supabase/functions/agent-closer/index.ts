...
Services recommandés: ${(p.recommended_services ?? []).join(", ") || "—"}
Note IA: ${p.ai_note ?? "—"}
Dirigeant: ${p.dirigeant ?? "—"}`;

    const { parsed, provider } = await callAI({
      provider: "auto",
      systemPrompt: "Tu es un closer SaaS/SMMA d'élite. Tu écris des scripts d'appel ULTRA personnalisés, courts, percutants, basés sur la psychologie de Chris Voss et Jordan Belfort. Pas de blabla.",
      userPrompt: `${objective ? `Objectif de l'appel: ${objective}\n` : ""}Prépare un script d'appel${tone ? ` (ton: ${tone})` : ""} pour closer ce prospect:
${ctx}`,
      tool,
